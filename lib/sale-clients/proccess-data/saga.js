...
module.exports = function (emitter, clientId, fileData, countprocess, callback) {
    ...
    if (fileName.reportdata != null && fileName.idsales != undefined && fileName.namesales != null && fileName.namesales != undefined && fileName.reportdata != null) {
        ...
        db.VentaEfectiva.max('codVenta').then(function (codmaxsale) {
            codmaxsaleeffective = codmaxsale;
                ...
                if (result != null) {
                    ...
                    searchPeriodMin(codretail).then(function (period) {
                        if (period.exists) {
                            ...
                            /**Buscar si existe alguna venta efectiva que pertenesca al ultimo periodo encontrado
                            ...
                            searchSaleEffectivePeriod(codperiod, codretail).then(function (saleinfo) {
                                if (saleinfo.exists) {
                                    ...
                                    searchPeriodByCodPeriod(saleinfo.codperiod).then(function (periodinfo) {
                                        ...
                                        var dia_periodo = fechafinperiod.getDay();
                                        var mes_periodo = fechafinperiod.getMonth();
                                        var anio_periodo = fechafinperiod.getFullYear();
                                        ...
                                        if (fecha_ayer_format < fecha_fin_periodo_format) {
                                            ...
                                            searchstore(codretail, namestore).then(function (storeinfo) {
                                                ...
                                                readDescriptionProducts(datasplit).then(function (products) {
                                                    ...
                                                    readProducts(products, storeinfo.codstore, function () {
                                                        callback();
                                                    });
...
function readDescriptionProducts(data) {
...
function readProducts(products, codstore, callback) {

    console.log("*** readProducts: where codProducts:" + products.length + " codStore:" + codstore);
    ...
    products.forEach(function (productssale) {
        ...
        console.log("**** readProducts FOREACH :" + tmpdescrip + " Ventas: " + tmpSaleperiod);

        /**Funcion que se sejecuta automaticamente, recibiendo datos para mantener los datos de las busqueda actual*/
        (function (saleperiod, descripproduct, codstore) {

            console.log("**** readProducts FOREACH FUNCTION :" + descripproduct + " Ventas: " + saleperiod + " RETAIL: " + codstore);
            /**Buscar productos en la tabla productos, pertenecientes a los retail, los cuales contienen un codigo
             * de equivalencia el cual sirve para saber que productos puenden estar asociados a los productos
             * de oxford
             * Nota*: Esta forma de realizar las equivalencias es por el problema de los codigos sin formato unicos
             * que tienen Proveedor: Oxford y Cliente: Reatils */
            searchProductRetail(codretail, descripproduct).then(function (productretail) {
                console.log("*** searchProductRetail Result// : %j", productretail);
                /**Verificamos si el productos que se busca tiene equivalencias, para poder indentificar los productos de oxford*/
                var codeq = productretail.codEq;

                console.log("** searchProductRetail CodEq: " + codeq);

                if (!productretail.exists) {
                    var productosdescripcion = productretail.nameProduct;

                    if (productoseqnotfound.indexOf(productosdescripcion) == -1) {
                        productosnotfound.push(productretail.nameProduct);
                    }
                }
                ...
                if (productretail.codEq == null) {
                    ...
                    (function (codEq, codRetail, codStore) {

                        console.log("** searchProductRetail FUNCTION ** CodEq: " + codEq);

                        /**Buscara en la tabla de productos pertenecientes a Oxford, usara el codigo de equivalencia obtenido anteriormente de los productos retails con lo cual obteneter su id, precio, linea, etc...*/
                        searchProduct(codEq).then(function (product) {
                            console.log("*** searchProduct: Result: %j", productretail);
                            if (product.idprod == null || product.idprod == '') {
                                console.log("PRODUCTO SIN IDE " + productretail.codEq + " " + product.idprod + " " + product.descrip);
                            } else {
                                /**Realizara una busqueda de la venta efectiva para poder saber si existe (Modificar) , si no (Crear)*/
